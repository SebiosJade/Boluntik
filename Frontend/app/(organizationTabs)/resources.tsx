...
export default function ResourcesScreen() {
  ...
  const handleRequestResource = async (resource: resourceService.Resource) => {
    try {
      ...
      webAlert('Success', 'Request sent successfully! You can track it in "Requested From Others" tab.');
  ...
  const handleOfferHelp = async (resource: resourceService.Resource) => {
    try {
      ...
      webAlert('Success', 'Offer sent successfully! You can track it in "Help I\'ve Offered" tab.');
      setInteractionMessage('');
      loadData();
    } catch (error: any) {
  ...
  const handleAcceptInteraction = async (interactionId: string) => {
    try {
      ...
      webAlert('Success', 'Request accepted! Resource marked as fulfilled.');
      setInteractionsModalVisible(false);
      setSelectedResource(null);
      loadData();
    } catch (error: any) {
      console.error('Error accepting interaction:', error);
      webAlert('Error', error.response?.data?.message || 'Failed to accept request');
    }
  };
  ...
  const handleDeclineInteraction = async (interactionId: string) => {
    try {
      ...
      webAlert('Success', 'Request declined');
      loadData();
    } catch (error: any) {
      console.error('Error declining interaction:', error);
      webAlert('Error', error.response?.data?.message || 'Failed to decline request');
    }
  };
  ...
  const renderResourceCard = (resource: resourceService.Resource, showActions: boolean = true) => {
    ...
    <View key={resource._id} style={[
      ...
      {showActions && renderActions(resource)}
    </View>
    );
  };
      ...
      <Animated.View style={[styles.sidebar, { transform: [{ translateX: slideAnim }] }]}>
        ...
        <View style={styles.menuContainer}>
          {menuItems.map((item) => (
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                if (item.id === 'dashboard') {
                  ...
                  // Already on resources, just close menu
            ...
            >
              <Ionicons name={item.icon as any} size={24} color={item.id === 'resources' ? '#3B82F6' : '#374151'} />
              <Text style={[styles.menuItemText, item.id === 'resources' && styles.activeMenuItemText]}>
                {item.title}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <ProfileDropdown showMenuButton={true} onMenuPress={toggleMenu} />
        ...
        <View style={styles.contentHeader}>
          <View style={styles.titleSection}>
            ...
            <Text style={styles.subtitle}>Share resources or request what you need</Text>
          </View>
        ...
        {(activeTab === 'browseOffers' || activeTab === 'browseRequests') && (
          <View style={styles.filterContainer}>
            ...
            >
              ...
              >
                Show only my {activeTab === 'browseOffers' ? 'offers' : 'requests'}
            </Text>
          </TouchableOpacity>
        </View>
        )}
