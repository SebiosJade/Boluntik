...
export default function PlatformWideAnalytics() {
  ...
  const [lastFetchedTimeframe, setLastFetchedTimeframe] = useState<Timeframe | null>(null);
  ...
  const getTimeframeFilters = (timeframe: Timeframe) => {
    const now = new Date();
  ...
  const loadAnalyticsData = async (forceRefresh = false) => {
    ...
    if (
      !forceRefresh &&
      analyticsData &&
      lastFetchedTimeframe === selectedTimeframe &&
    ...
    try {
      setLoading(true);
      setError(null);
      const timeframeFilters = getTimeframeFilters(selectedTimeframe);
      // Load all analytics data in parallel for faster performance
      const [usageData, revenueData, systemData] = await Promise.all([
        getUsageAnalytics(token, timeframeFilters),
        getRevenueAnalytics(token, timeframeFilters),
      ...
      setLastFetchedTimeframe(selectedTimeframe);
    } catch (err) {
        ...
        <View style={styles.menuContainer}>
          {menuItems.map((item) => (
            <TouchableOpacity
              ...
              style={[
                styles.menuItem,
                item.id === 'home' && styles.activeMenuItem,
              ]}
            ...
            >
              <Ionicons
                ...
                size={20}
                color={item.id === 'home' ? '#8B5CF6' : '#374151'}
              />
              <Text
                style={[
                  styles.menuItemText,
                  item.id === 'home' && styles.activeMenuItemText,
                ]}
              >
                {item.title}
              </Text>
